...
s2img_list = list.files("data/rasters/satellite_wheat", pattern = ".tif$", full.names = TRUE)
s2img_stack = stack(s2img_list)

# read DEM and create geomorphological variables
DEM = raster("data/rasters/DEM_wheat.tif")
...
names(morfo_stack) = c("DEM", "slope", "TPI", "TRI", 
                       "roughness", "northness", "eastness")
...
spplot(pred)
#writeRaster(pred, "wheat_results.tiff")
...
# Results:
# R^2 = 0.54  MAPE = 19.04%  RMSE = 23.70
...
# save
saveRDS(pred_df, "figures/data/wheatRfPred.RDS")
saveRDS(spatialDiff, "figures/data/wheatRfSpErr.RDS")